...
import net.eshop.dao.ReviewDao;
...
public class ReviewDaoImpl extends BaseDaoImpl<Review, Long> implements ReviewDao
{
	...
	public List<Review> findList(Member member, Product product, Type type, Boolean isShow, Integer count, List<Filter> filters,
			List<Order> orders)
	{
	...
	public boolean isReviewed(Member member, Product product)
	{
		...
		Long count = entityManager.createQuery(jqpl, Long.class).setFlushMode(FlushModeType.COMMIT).setParameter("member", member)
	...
	public long calculateScoreCount(Product product)
	{
		...
		return entityManager.createQuery(jpql, Long.class).setFlushMode(FlushModeType.COMMIT).setParameter("product", product)
				.setParameter("isShow", true).getSingleResult();
	}
