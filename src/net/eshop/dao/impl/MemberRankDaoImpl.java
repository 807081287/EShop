...
import net.eshop.dao.MemberRankDao;
...
public class MemberRankDaoImpl extends BaseDaoImpl<MemberRank, Long> implements MemberRankDao
{
	...
	public boolean nameExists(String name)
	{
		...
		Long count = entityManager.createQuery(jpql, Long.class).setFlushMode(FlushModeType.COMMIT).setParameter("name", name)
				.getSingleResult();
		return count > 0;
	}
	...
	public boolean amountExists(BigDecimal amount)
	{
		...
		Long count = entityManager.createQuery(jpql, Long.class).setFlushMode(FlushModeType.COMMIT).setParameter("amount", amount)
				.getSingleResult();
		return count > 0;
	}
	...
	public MemberRank findByAmount(BigDecimal amount)
	{
		...
		return entityManager.createQuery(jpql, MemberRank.class).setFlushMode(FlushModeType.COMMIT).setParameter("amount", amount)
	...
	public MemberRank merge(MemberRank memberRank)
	{
		...
		if (memberRank.getIsDefault())
		{
			...
			entityManager.createQuery(jpql).setFlushMode(FlushModeType.COMMIT).setParameter("memberRank", memberRank)
					.executeUpdate();
		}
	...
	public void remove(MemberRank memberRank)
	{
		if (memberRank != null && !memberRank.getIsDefault())
		{
			...
			List<Product> products = entityManager.createQuery(jpql, Product.class).setFlushMode(FlushModeType.COMMIT)
