...
public class MemberDaoImpl extends BaseDaoImpl<Member, Long> implements MemberDao
{
	...
	public boolean emailExists(String email)
	{
		...
		Long count = entityManager.createQuery(jpql, Long.class).setFlushMode(FlushModeType.COMMIT).setParameter("email", email)
				.getSingleResult();
		return count > 0;
	}
	...
	public List<Member> findListByEmail(String email)
	{
		...
		return entityManager.createQuery(jpql, Member.class).setFlushMode(FlushModeType.COMMIT).setParameter("email", email)
				.getResultList();
	}
	...
	public List<Object[]> findPurchaseList(Date beginDate, Date endDate, Integer count)
	{
		...
		criteriaQuery.multiselect(member.get("id"), member.get("username"), member.get("email"), member.get("point"),
				member.get("amount"), member.get("balance"), criteriaBuilder.sum(orders.<BigDecimal> get("amountPaid")));
		...
		restrictions = criteriaBuilder.and(restrictions, criteriaBuilder.equal(orders.get("orderStatus"), OrderStatus.completed),
