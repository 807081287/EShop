...
import net.eshop.dao.ReceiverDao;
...
public class ReceiverDaoImpl extends BaseDaoImpl<Receiver, Long> implements ReceiverDao
{
	...
	public Receiver findDefault(Member member)
	{
		...
		try
		{
			...
			return entityManager.createQuery(jpql, Receiver.class).setFlushMode(FlushModeType.COMMIT).setParameter("member", member)
					.getSingleResult();
		}
	...
	public void persist(Receiver receiver)
	{
		...
		if (receiver.getIsDefault())
		{
			...
			entityManager.createQuery(jpql).setFlushMode(FlushModeType.COMMIT).setParameter("member", receiver.getMember())
					.executeUpdate();
		}
